(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('reinsuranceApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-reinsuranceApp-error');
                var entityKey = httpResponse.headers('X-reinsuranceApp-params');
                if (errorHeader) {
